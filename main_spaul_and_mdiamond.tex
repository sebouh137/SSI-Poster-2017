...
\label{default}
\end{center}
\caption{(left) Dark matter self-interaction, which may solve ``core-cusp" anomalies in galactic haloes.  (right) Dark matter annihilating into leptons through $A'$, as a solution to cosmic-ray anomalies.}
\end{figure}
...
%	Beam backgrounds are significant because of our high beam current and forward detector coverage.
	...
	\begin{figure}
		\begin{center}
			%\includegraphics[width=1.2\textwidth]{HPS_detector_prop2014}
                        \includegraphics[width=1.05\textwidth]{hps_detector}
	...
	\begin{figure}
		\begin{center}
			\includegraphics[width=1.05\textwidth]{svt_2014_labelled}
	...
	\begin{figure}
		\begin{center}
			\includegraphics[width=0.9\textwidth]{ECal_looking_downstream}
...
\item A resonance search is sensitive at large values of $\alpha'/\alpha$, where the A' production rate is high.
...
%\end{itemize}
	...
	\begin{figure}
		\begin{flushright}
			...
			\includegraphics[width=1.2\textwidth]{reach/reach_new.pdf}
		\end{flushright}
		\caption{Anticipated reach in $\alpha'/\alpha=\epsilon^2$ for the HPS experiement using existing data samples (solid lines) and larger future data samples (dashed).  
		
 	Shaded regions represent existing constraints on the A' from various other experiments, including beam-dumps, other resonance searches and anomalous magnetic moments of electrons and muons.  
}
