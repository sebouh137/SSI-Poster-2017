...
%	Beam backgrounds are significant because of our high beam current and forward detector coverage.
	...
	Each layer has two closely spaced planes of silicon microstrip sensors to measure both X and Y coordinates for momentum measurement and track identification. Linear shift motion system allows SVT layers 1-3 to be moved within 15 mrad of the beam.
	...
	\begin{figure}
		\begin{center}
			\includegraphics[width=0.95\textwidth]{ECal}
                        %\includegraphics[width=0.47\textwidth]{ecal_module}
		\end{center}
		\caption{A rendering of the electromagnetic calorimeter setup looking down the beam line (left) and a more detailed view of the Ecal crystal. The Ecal contains 442 lead tungstate crystals.
...
%	A level 1 hardware trigger selects events to be read out. The triggered events are acquired and processed in the data acquisition and processing system. The DAQ system is designed for a maximum trigger rate of 50 kHz; simulations estimate that the ECal trigger rate due to background will be approximately 17 kHz.
	...
	\begin{figure}
		...
		JLab (brown lines) with \emph{existing} constraints on an $A'$ from electron and muon anomalous magnetic
		moment measurements (green), the BaBar search for $\gamma(3S)\to\gamma\mu^+\mu^-$, and several beam dump experiments. %Red is the reach for the 2015 HPS engineering run and blue is the expected reach for the full data set.}
	\end{figure}
...
%	\end{figure}
        ...
        The major physics results of HPS will result from the 2018 running. %and will produce several theses. HPS is currently looking for 
%        students (both rotation and permanent) for upgrades, data taking, and data analysis.
        Several upgrades have been proposed in order to improve the reach of the experiment:  
     	\begin{itemize}
	\item Including an additional layer to the Silicon Vertex Tracker, halfway between the target and the existing first layer of the tracker.  This would improve both the vertex position resolution and the mass resolution.  
	\item Adding a hodoscope on the side where the positrons curve towards, which would be used for an additional positron-only trigger.  This upgrade would increase the geometric acceptance of the detector.  
	\end{itemize}
	\section{Citations}
	Blah
